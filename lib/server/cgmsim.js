const { main, surprise } = require('cgmsim-lib')
...
function init (env, ctx) {
  ...
  cgmsim.accessToken = (code) => {
    if (code) {
      ...
      return google.getAccesstoken(code, client_id, client_secret, code_verifier, redirect_uri)
        .then((res) => {
          console.log('####', res);
  ...
  const saveParams = (cgmsimEnv) => {
    ...
    api().updateOne({ type: PARAMS_TYPE }, {
      $set: {
        type: PARAMS_TYPE
        , ISF
        , CR
    ...
    if (access_token) {
      ...
      google.getFitSteps(access_token, from, to)
        .then((res) => {
          ...
          const listSteps = []
          res.bucket.forEach(b => {
            console.log('@@@@@FIT', JSON.stringify(b))
            let steps = 0;
            if (b.dataset[0].point.length && b.dataset[0].point[0].value.length) {
              steps = b.dataset[0].point[0].value[0].intVal;
            }
            listSteps.push({
              created_at: moment(parseInt(b.endTimeMillis)).toISOString()
              , steps
            });
          ...
          //   }
          ctx.activity.create(listSteps, function stored (err) {
            if (err) {
  ...
  cgmsim.code_verifier = function(postEnv) {
    code_verifier = postEnv.code_verifier;
  }
  cgmsim.start = function(postEnv) {
    if (postEnv) {
  ...
  cgmsim.get = function start () {
    ...
    const isGoogleActive = !!googleToken.refresh_token;
    return { ISF, CR, DIA, WEIGHT, TP, CARBS_ABS_TIME, pumpEnabled, surpriseEnabled, planetEnabled, isActive, isGoogleActive };
  }
