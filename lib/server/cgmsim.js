...
function init (env, ctx) {
  ...
  cgmsim.start = function start (postEnv) {
    if (postEnv) {
      updateParams(cgmsimEnv, { ...postEnv, isActive: cgmsimEnv.isActive });
    }
    // setInterval(() => {
    //   clearPumpBasals();
    // }, 10000)

    if (cgmsimEnv.isActive === false) {
  ...
  function updateParams (cgmsimEnv, postEnv) {
  ...
  function loadSettings (cgmsimEnv) {
