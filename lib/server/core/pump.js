// const mongo = require('./mongo.js');
// const moment = require('moment');

//require('dotenv').config();

// const iobTotal= require('./iobTotal');
// const result = iobTotal(basalAsBoluses,dia);
// console.log(result);

module.exports = async function ({
	entries,
	profile,
},env) {
	// let storedBasals;
	// let cgmsimDbGet;
	// let cgmsimClient;
	// const dia = parseInt(env.DIA);
	// const basalAsBoluses = [];
	// const endDiaAction = moment();
	// const startDiaAction = moment().add(-dia, 'hour').set({minute:0,second:0,millisecond:0});

	// try {
	// 	const _mongo = await mongo(env);
	// 	cgmsimClient = _mongo.client;
	// 	cgmsimDbGet = _mongo.db;
	// 	const basalsCollection = await cgmsimDbGet.collection('basals');
	// 	storedBasals = await basalsCollection.find({
	// 		date: {
	// 			$gte: startDiaAction.toDate()
	// 		}
	// 	}, {
	// 		date: null,
	// 		insulin: null
	// 	}).toArray();

	// } catch (error) {
	// 	console.log(error);
	// } finally {
	// 	await cgmsimClient.close();

	// }

	// let lastProfile = profile.filter(e => e.store.Default && e.store['Default'])
	// 	.sort((first, second) => second.startDate - first.startDate)[0];
	// if (!lastProfile) {
	// 	return 0;
	// }
	// const defaultProfileBasals = lastProfile.store.Default.basal;

	// // here we start gathering the tempBasals from the treatment.json file
	// //====================================================================

	// const now = moment();
	// const allTempBasals = entries
	// 	.filter(e =>
	// 		e._id &&
	// 		e.create_at &&
	// 		moment(e.create_at).diff(now, 'minutes') <= (60 * dia) && // temps basals set in the last 3 hours
	// 		e.eventType === 'Temp Basal' &&
	// 		e.duration != 0
	// 	)
	// 	.sort((f, s) => {
	// 		return moment(f.create_at).diff(s.create_at);
	// 	});
	// //compute fasal from entries
	// const computedTempBasal = [];
	// allTempBasals.forEach((b) => {
	// 	if (b.absolute !== undefined) {
	// 		const start = moment(b.create_at);
	// 		computedTempBasal.push({
	// 			start,
	// 			insulin: b.absolute,
	// 			end: start.add(b.duration, 'minutes')
	// 		});
	// 	} else {
	// 		const currentIndex = computedTempBasal.length - 1;
	// 		if (currentIndex >= 0) {
	// 			computedTempBasal[currentIndex].end = moment(b.create_at);
	// 		}
	// 	}
	// });


	// const basalsToUpdate = [];
	// for (let currentAction = startDiaAction; currentAction.diff(endDiaAction) <= 0; currentAction.add(5, 'minutes')) {
	// 	const storedBasal =  storedBasals.find(b =>  currentAction.diff(moment(b.date))<1000);
	// 	if (storedBasal) {
	// 		basalAsBoluses.push({
	// 			...storedBasal
	// 		});
	// 	} else {
	// 		const tempBasalActives = allTempBasals
	// 			.filter(t => {
	// 				t.start.diff(currentAction) <= 0 &&
	// 					t.end.diff(currentAction) > 0;
	// 			});
	// 		let basalToUpdate;
	// 		//if there is a temp basal actives
	// 		if (tempBasalActives.lengt > 0) {
	// 			basalToUpdate = {
	// 				date: currentAction.toDate(),
	// 				time: currentAction.toDate(),
	// 				insulin: tempBasalActives.insulin / 12,
	// 			};
	// 		} else {
	// 			const compatiblesBassalProfiles = defaultProfileBasals.filter(b => {
	// 				return b.time.localeCompare(currentAction.format('HH:mm')) <= 0;
	// 			});
	// 			const index = compatiblesBassalProfiles.length - 1;
	// 			const currentBasal = compatiblesBassalProfiles[index];

	// 			basalToUpdate = {
	// 				date: currentAction.toDate(),
	// 				time: currentAction.toDate(),
	// 				insulin: currentBasal.value / 12,
	// 			};
	// 		}
	// 		basalsToUpdate.push(basalToUpdate);
	// 		basalAsBoluses.push(basalToUpdate);
	// 	}
	// }


	// let cgmsimDbInsert;
	// let cgmsimClientInsert;
	// if(basalsToUpdate && basalsToUpdate.length>0){
	// 	try {
	// 		const mongo = await mongo();
	// 		cgmsimClientInsert = mongo.client;
	// 		cgmsimDbInsert = mongo.db;
	// 		const basalsCollection = await cgmsimDbInsert.collection('basals');
	// 		const options = {
	// 			ordered: true
	// 		};
	// 		await basalsCollection.insertMany(basalsToUpdate, options);		
	// 	} catch (error) {
	// 		console.log(error);
	// 	} finally {
	// 		await cgmsimClientInsert.close();
	// 	}	
	// }

	// var td = dia * 60;
	// var tp = parseInt(env.TP);
	// var tau = tp * (1 - tp / td) / (1 - 2 * tp / td);
	// var a = 2 * tau / td;
	// var S = 1 / (1 - a + (1 + a) * Math.exp(-td / tau));


	// let timeSincePumpAct = basalAsBoluses.map(entry => {
	// 	var t = now.diff(entry.time, 'minute');
	// 	var insulin = entry.insulin;
	// 	return {
	// 		...entry,
	// 		time: t,
	// 		activityContrib: insulin * (S / Math.pow(tau, 2)) * t * (1 - t / td) * Math.exp(-t / tau)
	// 	};
	// });

	// // console.log('basal as boluses with detailed activities:', timeSincePumpAct);



	// let lastPumpInsulins = timeSincePumpAct.filter(function (e) {
	// 	return e.time <= (dia * 60 * 1000);
	// });
	// console.log('these are the last pump basal insulins and detailed activities:', lastPumpInsulins);


	// var resultPumpAct = lastPumpInsulins.reduce(function (tot, arr) {
	// 	return tot + arr.activityContrib;
	// }, 0);
	// console.log('this is the aggregated insulin activity from pump basal in the last dia hours:', resultPumpAct);


	// const pumpBasalAct = JSON.stringify(resultPumpAct, null, 4);
	// console.log('the pump´s basal activity is:', pumpBasalAct);
	// // fs.writeFile('./files/pumpBasalAct.json', pumpBasalAct, (err) => {
	// // 	if (err) {
	// // 		throw err;
	// // 	}
	// // 	console.log('Pump basal activity is now is saved as JSON.');
	// // });
	// return Math.round(resultPumpAct * 100000) / 100000;
};