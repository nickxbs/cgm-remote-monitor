...
const uploadCgmsimRun = require('./upload-cgmsim.js');

console.log('Run Init');

const main =  ({ env, entries, treatments, profile }) => {
  const weight = parseInt(env.WEIGHT);

  const perls = perlinRun();
  //   const { treatments, profile, entries } = await downloadsRun(env);
  ...
  const lastMeals = allMealsRun(entries);
  const carbs = carbsRun(lastMeals);
  const arrows = arrowsRun(entries);
