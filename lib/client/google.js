...
const getChallenge = (verifier) => base64URLEncode(sha256(verifier));
...
function openGoogleSignIn (client_id, redirect_uri, code_challenge) {
  ...
  const generateAuthUrl = (opts) => {
    ...
    opts.code_challenge = code_challenge;
...
const getFitSteps = (token, fromMillis, toMillis) => {
  ...
  const data = {
    ...
    , bucketByTime: { "durationMillis": (toMillis - fromMillis) / 12 }
  ...
  , }
  console.log('@@@Fit data', JSON.stringify(data))
  console.log('@@@Fit headers', JSON.stringify(headers))
  return fetch(url, {
      method: 'POST'
      , headers
...
const getAccesstoken = (code, client_id, client_secret, code_verifier, redirect_uri) => {
  if (code) {
    const url = 'https://oauth2.googleapis.com/token';
